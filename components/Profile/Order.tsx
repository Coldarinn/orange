    ...
    <div className="w-full bg-white rounded-[20px] border border-stroke-dark py-[24px] mb-[28px]">
      <div className="flex justify-between items-center px-[24px] pb-[16px]">
        <div className="flex items-center text-xl">
          ...
          <div className="text-text-700 pt-sans ml-[32px]">
            {order.number}
          </div>
        </div>
        <div>
          <div className={`text-xl text-right ubuntu${order.status < 5 ? ' text-brand-700' : ' text-success'}`}>
            {status.title}
          </div>
          {status.text && (
          <div className="text-text-500 text-sm mt-[8px]">
            {status.text}
          </div>
          )}
        </div>
      ...
      <div className="px-[24px] pb-[16px] pt-[24px] border-t border-stroke-dark">
        <div className="text-xl font-medium mb-[32px]">
          Самовывоз
        </div>
        <div className="relative flex items-center justify-between mb-[80px]">
          {orderStatuses.map((item, idx) => (
            ...
            >
              <span className={`relative z-[2] mb-[10px] flex items-center justify-center w-[32px] h-[32px] rounded-full font-bold ${order.status > idx ? 'bg-success text-white' : 'bg-text-200 text-text-600'}`}>
                {idx + 1}
                <span
                  className={`absolute bottom-[-40px] text-text-800 font-normal whitespace-nowrap ${idx === 0 ? 'left-0 text-left' : idx === 4 ? 'right-0 text-right' : 'left-1/2 -translate-x-1/2 text-center'}`}
              ...
              && (
              <div
                className={`absolute top-[16px] w-1/4 h-[2px] z-[1] ${order.status > idx + 1 ? 'bg-success' : 'bg-text-200'}`}
        ...
        <div className="flex">
          <div className="basis-1/2">
            <div className="flex text-text-600 mb-[16px]">
              <div className="basis-[111px] font-medium">
                Статус оплаты:
              </div>
          ...
          <div className="basis-1/2">
            <div className="flex text-text-600 mb-[16px]">
              <div className="basis-[122px] font-medium">
                Срок доставки:
              </div>
              <div className="ml-[32px]">
                {order.deliveryDate}
      ...
      <div className="px-[24px] mt-[16px]">
        ...
        >
          <Arrow className={`mr-[12px] stroke-black ${isOpen ? 'rotate-180' : 'rotate-0'}`} />
          Детали
        </button>
      </div>
    </div>
